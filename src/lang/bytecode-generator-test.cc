...
TEST_F(BytecodeGeneratorTest, RunNumberInboxUnbox) {
    HandleScope handle_scope(HandleScope::INITIALIZER);

    auto rs = isolate_->Compile("tests/lang/025-number-inbox-unbox");
    ASSERT_TRUE(rs.ok()) << rs.ToString();
...
TEST_F(BytecodeGeneratorTest, RunNumberArithmetic) {
    HandleScope handle_scope(HandleScope::INITIALIZER);

    auto rs = isolate_->Compile("tests/lang/026-number-arithmetic");
    ASSERT_TRUE(rs.ok()) << rs.ToString();
...
TEST_F(BytecodeGeneratorTest, RunNumberComparation) {
    HandleScope handle_scope(HandleScope::INITIALIZER);

    auto rs = isolate_->Compile("tests/lang/027-number-comparation");
    ASSERT_TRUE(rs.ok()) << rs.ToString();
...
TEST_F(BytecodeGeneratorTest, CallingSanity) {
    Define("028-calling-sanity");
    HandleScope handle_scope(HandleScope::INITIALIZER);
...
TEST_F(BytecodeGeneratorTest, RunCallingSanity) {
    HandleScope handle_scope(HandleScope::INITIALIZER);

    auto rs = isolate_->Compile("tests/lang/028-calling-sanity");
    ASSERT_TRUE(rs.ok()) << rs.ToString();
...
TEST_F(BytecodeGeneratorTest, RunTestAsIs) {
    HandleScope handle_scope(HandleScope::INITIALIZER);

    auto rs = isolate_->Compile("tests/lang/029-type-test-as-is");
    ASSERT_TRUE(rs.ok()) << rs.ToString();
...
TEST_F(BytecodeGeneratorTest, WhenExpression) {
    Define("031-when-expression");
    HandleScope handle_scope(HandleScope::INITIALIZER);
    ...
    auto value = generator_->FindValue("main.switchType");
    Local<Closure> fun(*isolate_->global_offset<Closure *>(value.index));
    ASSERT_TRUE(fun.is_value_not_null());
    ASSERT_TRUE(fun->is_mai_function());
    ASSERT_FALSE(fun->is_cxx_function());
    AssertFunction("switchType", fun->function());
    
    value = generator_->FindValue("main.switchSize");
    fun = *isolate_->global_offset<Closure *>(value.index);
    ASSERT_TRUE(fun.is_value_not_null());
    ASSERT_TRUE(fun->is_mai_function());
    ASSERT_FALSE(fun->is_cxx_function());
...
TEST_F(BytecodeGeneratorTest, RunWhenExpression) {
    ...
    auto rs = isolate_->Compile("tests/lang/031-when-expression");
    ASSERT_TRUE(rs.ok()) << rs.ToString();
...
TEST_F(BytecodeGeneratorTest, ForeachArray) {
    Define("032-foreach-array");
    HandleScope handle_scope(HandleScope::INITIALIZER);
    ...
    auto value = generator_->FindValue("main.assertForeachArraySanity");
    Local<Closure> fun(*isolate_->global_offset<Closure *>(value.index));
    ASSERT_TRUE(fun.is_value_not_null());
    ASSERT_TRUE(fun->is_mai_function());
    ASSERT_FALSE(fun->is_cxx_function());
    ...
    ASSERT_TRUE(fun->is_mai_function());
    ASSERT_FALSE(fun->is_cxx_function());
    AssertFunction("assertForeachArrayIndexed", fun->function());
}

TEST_F(BytecodeGeneratorTest, RunForeachArray) {
    HandleScope handle_scope(HandleScope::INITIALIZER);

    auto rs = isolate_->Compile("tests/lang/032-foreach-array");
    ASSERT_TRUE(rs.ok()) << rs.ToString();
