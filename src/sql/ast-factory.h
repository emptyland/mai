...
public:
    ...
    // DDL
    ////////////////////////////////////////////////////////////////////////////////////////////////
    CreateTable *NewCreateTable(const Identifier *table_name, ColumnDefinitionList *cols) {
        return new (arena_) CreateTable(table_name, cols);
    }
    
    AlterTable *NewAlterTable(const Identifier *table_name, AlterTableSpecList *spces) {
    ...
    AlterTableColumn *NewAlterTableAddColumn(ColumnDefinition *col, bool after,
    ...
    AlterTableColumn *NewAlterTableChangeColumn(const AstString *old_col_name,
                                                ColumnDefinition *col, bool after,
                                                const AstString *col_name) {
        auto cols = NewColumnDefinitionList(col);
    ...
    AlterTableIndex *NewAlterTableAddIndex(const AstString *index_name, SQLKeyType key,
                                           NameList *col_names) {
        return new (arena_) AlterTableIndex(index_name, key, col_names);
    }
    
    AlterTableIndex *NewAlterTableDropIndex(const AstString *drop_idx_name, bool primary_key) {
    ...
    ColumnDefinition *NewColumnDefinition(const AstString *name, TypeDefinition *type) {
        return new (arena_) ColumnDefinition(name, type);
    }
    
    TypeDefinition *NewTypeDefinition(SQLType type, int d_size = 0, int m_size = 0) {
        return new (arena_) TypeDefinition(type, d_size, m_size);
    }
    
    ////////////////////////////////////////////////////////////////////////////////////////////////
    // TCL
    ////////////////////////////////////////////////////////////////////////////////////////////////
    TCLStatement *NewTCLStatement(TCLStatement::Txn cmd) { return new (arena_) TCLStatement(cmd); }
    
    ////////////////////////////////////////////////////////////////////////////////////////////////
    // DML
    ////////////////////////////////////////////////////////////////////////////////////////////////
    Select *NewSelect(bool distinct, ProjectionColumnList *cols, const AstString *alias) {
    ...
    JoinRelation *NewJoinRelation(Query *lhs, SQLJoinKind join, Query *rhs, Expression *on_expr,
    ...
    Update *NewUpdate(const Identifier *table_name, AssignmentList *assignments) {
    ...
    // Expressions
    ////////////////////////////////////////////////////////////////////////////////////////////////
    ProjectionColumn *NewProjectionColumn(Expression *expr, const AstString *alias,
    ...
    Identifier *NewIdentifierWithPlaceholder(const AstString *prefix, Placeholder *placeholder,
                                             const Location &location) {
    ...
    Literal *NewDateLiteral(SQLDate val, const Location &location) {
        return new (arena_) Literal({val, {0,0,0,0,0}}, Literal::DATE, location);
    }
    
    Literal *NewTimeLiteral(SQLTime val, const Location &location) {
        return new (arena_) Literal({{0, 0, 0}, val}, Literal::TIME, location);
    }
    
    Literal *NewDateTimeLiteral(SQLDateTime val, const Location &location) {
        return new (arena_) Literal(val, Literal::DATETIME, location);
    }
    ...
    Subquery *NewSubquery(bool match_all, Query *query, const Location &location) {
        return new (arena_) Subquery(match_all, query, location);
    }
    
    Comparison *NewComparison(Expression *lhs, SQLOperator op, Expression *rhs,
                              const Location &location) {
        DCHECK(Comparison::IsComparisonOperator(op));
        return new (arena_) Comparison(op, lhs, rhs, location);
    }
    
    MultiExpression *NewMultiExpression(Expression *lhs, SQLOperator op, ExpressionList *rhs,
                                        const Location &location) {
        return new (arena_) MultiExpression(op, lhs, rhs, location);
    }
    
    MultiExpression *NewMultiExpression(Expression *lhs, SQLOperator op, Expression *rhs,
                                        const Location &location) {
        ExpressionList *stub = NewExpressionList(rhs);
        return NewMultiExpression(lhs, op, stub, location);
    }
    
    MultiExpression *NewMultiExpression(Expression *lhs, SQLOperator op, Expression *operand1,
                                        Expression *operand2, const Location &location) {
        ExpressionList *stub = NewExpressionList(operand1);
        stub->push_back(operand2);
        return NewMultiExpression(lhs, op, stub, location);
    }
    
    BinaryExpression *NewBinaryExpression(Expression *lhs, SQLOperator op, Expression *rhs,
                                          const Location &location) {
    ...
    Assignment *NewAssignment(const AstString *name, Expression *rval, const Location &location) {
        return new (arena_) Assignment(name, rval, location);
    }
    
    Call *NewCall(const AstString *name, bool disinct, ExpressionList *parameters,
                  const Location &location);
    ...
    const AstString *NewString(const char *s, size_t n) { return AstString::New(arena_, s, n); }
    
    const AstString *NewString(const char *s) { return AstString::New(arena_, s); }
