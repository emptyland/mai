...
TEST_F(NyaaThreadAOTTest, MapMetaIndex) {
    HandleScope scope(N_);
    TryCatchCore try_catch(core_);
    ASSERT_EQ(0, NyClosure::DoFile("tests/nyaa/20-map-index-meta.nyaa", 0, nullptr, core_))
    << try_catch.ToString();
...
TEST_F(NyaaThreadAOTTest, GCTrigger) {
    HandleScope scope(N_);
    TryCatchCore try_catch(core_);
...
TEST_F(NyaaThreadAOTTest, ShiftOps) {
    HandleScope scope(N_);
    TryCatchCore try_catch(core_);
    ASSERT_EQ(0, NyClosure::DoFile("tests/nyaa/15-bit-shift.nyaa", 0, nullptr, core_))
        << try_catch.ToString();
    ASSERT_FALSE(try_catch.has_caught()) << try_catch.ToString();
}
    
TEST_F(NyaaThreadAOTTest, UnaryOps) {
    HandleScope scope(N_);
    TryCatchCore try_catch(core_);
    ASSERT_EQ(0, NyClosure::DoFile("tests/nyaa/17-unary-ops.nyaa", 0, nullptr, core_))
        << try_catch.ToString();
    ASSERT_FALSE(try_catch.has_caught()) << try_catch.ToString();
}
